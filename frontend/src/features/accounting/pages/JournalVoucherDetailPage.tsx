...
export const JournalVoucherDetailPage: React.FC = () => {
  ...
  return <div style={{ maxWidth: 1200, margin: '0 auto', padding: '1.5rem' }}>
    ...
    <PageHeader title={journal.journal_number} subtitle="Posted accounting voucher — read only" actions={<div style={{ display: 'flex', gap: 8 }}><button className="btn btn-secondary" onClick={() => window.print()}><Printer size={16}/> Print</button>{journal.status === 'posted' && !journal.reversal_of && <button className="btn btn-danger" onClick={reverse}><RotateCcw size={16}/> Reverse</button>}</div>}/>
    {error && <div className="alert alert-error">{error}</div>}
    <section className="card" style={{ padding: 20, marginBottom: 18 }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 16 }}><div><div className="text-muted">Date</div><strong>{journal.entry_date}</strong></div><div><div className="text-muted">Reference</div><strong>{journal.reference || '—'}</strong></div><div><div className="text-muted">Source</div><strong>{journal.source_type === 'manual_journal' ? 'Manual Journal' : 'Reversal'}</strong></div><div><div className="text-muted">Status</div><span className={`status-badge ${journal.status === 'posted' ? 'success' : 'danger'}`}>{journal.status}</span></div></div><div style={{ marginTop: 18 }}><div className="text-muted">Narration</div><strong>{journal.narration}</strong></div>{journal.reversal_reason && <div className="alert" style={{ marginTop: 16 }}>Reversal reason: {journal.reversal_reason}</div>}</section>
