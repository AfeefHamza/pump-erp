...
export const ChartOfAccountsPage: React.FC = () => {
  ...
  return <div style={{ maxWidth: 1500, margin: '0 auto', padding: '1.5rem' }}>
    ...
    {tab === 'accounts' ? <>
      ...
      {(trial?.rows || []).length === 0 ? <tr><td colSpan={7} style={{ textAlign: 'center', padding: 44 }}>No postings for this outlet.</td></tr> : accounts.filter((a) => trialByAccount.has(a.id)).map((account) => { const row = trialByAccount.get(account.id)!; return <tr key={account.id}><td><strong>{row.code}</strong></td><td>{row.name}</td><td>{labels[row.account_type]}</td><td style={{ textAlign: 'right' }}>{money(row.debit)}</td><td style={{ textAlign: 'right' }}>{money(row.credit)}</td><td style={{ textAlign: 'right' }}>{money(row.closing_debit)}</td><td style={{ textAlign: 'right' }}>{money(row.closing_credit)}</td></tr>; })}
