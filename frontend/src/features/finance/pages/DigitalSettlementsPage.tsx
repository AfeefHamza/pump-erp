...
export const DigitalSettlementsPage: React.FC = () => {
  ...
  return <div style={{ maxWidth: 1450, margin: '0 auto', padding: '1.5rem' }}><PageHeader title="Digital Settlements" subtitle="Match Shift Card collections to card, UPI and fleet-card credits received in the bank." actions={<button className="btn btn-primary" onClick={() => navigate('/app/finance/settlements/new')}><Plus size={16}/> New Settlement</button>}/>{error && <div className="alert alert-error">{error}</div>}
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 14, marginBottom: 18 }}>{[
      ['Digital Clearing Balance', `${summary?.pending_count || 0} · ${money(summary?.pending_amount || '0')}`], ['Gross Settled', money(summary?.settled_gross || '0')], ['Charges', money(summary?.charges_total || '0')], ['Net Bank Credit', money(summary?.net_received || '0')],
    ...
    {!!summary?.pending_by_provider?.length && <div className="card" style={{ padding: 16, marginBottom: 18 }}><div className="text-muted" style={{ marginBottom: 10 }}>Pending clearing by provider</div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>{summary.pending_by_provider.map((row) => <div key={`${row.collection_method}-${row.provider_name}`} style={{ border: '1px solid var(--border-color)', borderRadius: 8, padding: '10px 14px' }}><strong>{row.provider_name}</strong><div className="text-muted">{methodName(row.collection_method)} · {row.count} · {money(row.amount)}</div></div>)}</div></div>}
